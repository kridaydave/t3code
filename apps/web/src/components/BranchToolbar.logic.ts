...
// Only trigger presses arm the suppress window. A close from any other
// reason (item-press, outside-press, escape) disarms it, so picking an item
// never eats a deliberate reopen right after.
export function resolveNextBranchMenuToggleAt(input: {
  reason: string | undefined;
  open: boolean;
  lastToggleAt: number;
  now: number;
}): number {
  if (input.reason === "trigger-press") return input.now;
  if (!input.open) return 0;
  return input.lastToggleAt;
}
