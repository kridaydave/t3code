...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("remote operations", () => {
    ...
    it.effect("serializes concurrent pulls from a root and its subdirectory", () =>
      Effect.gen(function* () {
        const driver = yield* GitVcsDriver.GitVcsDriver;
        const fileSystem = yield* FileSystem.FileSystem;
        const pathService = yield* Path.Path;
        ...
        yield* git(clone, ["config", "user.email", "test@test.com"]);
        yield* git(clone, ["config", "user.name", "Test"]);
        const subdir = pathService.join(clone, "sub");
        ...
        const results = yield* Effect.all(
          [driver.pullCurrentBranch(clone), driver.pullCurrentBranch(subdir)],
          { concurrency: 2 },
        );
        assert.deepEqual(results.map((result) => result.status).sort(), [
          "pulled",
