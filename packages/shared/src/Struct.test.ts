...
describe("deepMerge", () => {
  ...
  it("recursively merges nested objects", () => {
    ...
    // base's nested object is not mutated in place
  ...
  it("merges multiple patch objects in sequence", () => {
    ...
    // base's nested object is not mutated across the sequence of merges
    expect(base.nested).toEqual({ x: 1, y: 2 });
  });

  it("skips keys with undefined values in the patch", () => {
    const base = { a: 1, b: 2 };
    // A literal `{ a: undefined }` is a type error under
    // exactOptionalPropertyTypes, so the undefined must come from a variable.
    // The cast documents the runtime path where a key resolves to undefined
    // and is skipped.
  ...
  it("returns a shallow copy of the base (nested refs shared — known limitation)", () => {
    ...
    // The top-level object is a new reference...
    expect(merged).not.toBe(base);
    // ...but nested objects are NOT cloned (shallow-copy contract): the nested
    // reference is shared with base.
